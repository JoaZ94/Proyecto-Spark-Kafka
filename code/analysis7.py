...
if __name__ == "__main__":
    ...
    def enviar_notificaciones(df, epoch_id):
        df.foreach(lambda row: enviar_notificacion(row.nombre, row.productovisto))

    # Aplicar la función enviar_notificaciones a cada lote de filas del DataFrame resultante
    clientes_notificados_df.writeStream \
        .outputMode("append") \
        .foreachBatch(enviar_notificaciones) \
        .start()
